...
namespace EshopModulith.Shared.Behavior
{
        ...
        where TResponse : notnull
    {
        public async Task<TResponse> Handle(TRequest request, RequestHandlerDelegate<TResponse> next, CancellationToken cancellationToken)
        {
            ...
            logger.LogInformation($"[END] Handle request {typeof(TRequest).Name} - Response {typeof(TResponse).Name} - TimeTaken = {timeTaken} - ResponseData = {response}");
